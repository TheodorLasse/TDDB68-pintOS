...
/* for debugging */
void printListProcess(struct list* printList){
  struct list_elem *e;
  ...
  for (e = list_begin(printList); e!= list_end(printList); e = list_next(e)){
    ...
    printf("prev: %p, next: %p \n\n", iterChild->elem.prev, iterChild->elem.next);
  }
}
...
process_execute (const char *cmd_line) 
{
  ...
  strlcpy (fn_copy, strtok_r (cmd_copy2, " ", &save_ptr), PGSIZE);
  ...
  lock_init(&(pc->pcLock));
  lock_acquire(&(pc->pcLock));
  pc->alive_count = 2;
  pc->exit_status = -1;
  lock_release(&(pc->pcLock));

  pc->cmd_line = cmd_copy;
  pc->file_name = fn_copy;

  sema_init(&(pc->waitSema), 0);
  sema_init(&(pc->exitWaitSema), 0);

  tid = thread_create(fn_copy, PRI_DEFAULT, start_process, pc);
  ...
  sema_down(&(pc->waitSema));
  
  if(pc->childId == TID_ERROR){
    tid = -1;
    pc->exit_status = -1;
  }
  else
  {
    list_push_back(&(thread_current()->childList), &(pc->elem));
  }
...
/* A thread function that loads a user process and starts it
   running. */
static void
start_process (void* pc)
{
  struct intr_frame if_;
  bool success;
  struct thread* cur = thread_current();
  cur->parent = (struct parent_child*)pc;
  char *file_name = cur->parent->file_name;
  ...
  /* If load failed, quit. */
  if (!success){
    cur->parent->childId = TID_ERROR;
    sema_up(&(cur->parent->waitSema));
    thread_exit ();
  }
  
  tid_t newChildId = cur->tid;
  cur->parent->childId = newChildId;
  cur->parent->exit_status = 0;
  sema_up(&(cur->parent->waitSema));
...
process_wait (tid_t child_tid) 
{
  ...
  {
    struct parent_child *iterChild = list_entry (e, struct parent_child, elem);
    if(child_tid == iterChild->childId)
    {
      lock_acquire(&(iterChild->pcLock));
      if(iterChild->alive_count == 2)
      {
        lock_release(&(iterChild->pcLock));
        sema_down(&(iterChild->exitWaitSema));
...
process_exit (void)
{
  ...
  struct list_elem *e;
  for (e = list_begin (&cur->childList); e != list_end (&cur->childList); e = list_next (e))
  {
    struct parent_child *iterChild = list_entry (e, struct parent_child, elem);
    lock_acquire(&iterChild->pcLock);
    iterChild->alive_count--;
    if (iterChild->alive_count == 0)
    {
  ...
  if (parent != NULL)
  {
    lock_acquire(&(parent->pcLock));
    parent->alive_count--;
    sema_up(&(parent->exitWaitSema));
    if (parent->alive_count == 0){
      lock_release(&(parent->pcLock));
      free(parent);
    }
    else{
      lock_release(&(parent->pcLock));
  ...
  if (kpage != NULL) 
    {
      ...
      if (success){
        *esp = PHYS_BASE;
        char* cmd_line = thread_current()->parent->cmd_line;
        ...
        void* arg_pointers[32];

        // Add args to stack and save pointers to them in arg_pointers.
        for(int j = i - 1; j >= 0; j--){
          char* token = args[j];
          int tokenLength = strlen(token) + 1;
          *esp -= tokenLength;
          arg_pointers[j] = memcpy(*esp, token, tokenLength);
        }
        uint8_t offset = (uint8_t)*esp;
        *esp -= offset%4;
        //printf("esp: %p, unsigned: %d",*esp, offset);

        *esp -= sizeof(char*);

        for (int j = i - 1; j >= 0; j--){
          *esp -= sizeof(char*);
          memcpy(*esp, &arg_pointers[j], sizeof(char*));
          //printf("esp: %p, str: %s, str_pointer: %p\n", *esp, arg_pointers[j], arg_pointers[j]);
        }

        char* argAdr = *esp;

        *esp -= sizeof(char**);
        memcpy(*esp, &argAdr, sizeof(char**));
        //printf("adr %p \n", &argAdr);
